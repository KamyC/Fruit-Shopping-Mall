...
function initialize() {
  ...
  //get string information from <main></main>
 ...
 var category = document.querySelector('#category');
  ...
  function fetchBlob(product) {
    ...
    var url = 'public/img/' + product.picture;
  ...
  function showProduct(objectURL, product) {
    ...
    function addtocart() {
      ...
      // get product price and assign to Defultprice
        ...
        {
  //Fuck finally it is working for calculate!!!!!!!*****************
          CalculatedPrice=Productquantity*DefultPrice;
              updatetoShoppingCart();
        }
        else
        {
          window.alert("Not avaiable, please re-input")
             NumberInpute.value="";
        }

    }

      function updatetoShoppingCart()
      {
                  var cart_inputbuttonadd = document.createElement("button");
                  var cart_inputbuttonmins = document.createElement("button");
                  cart_inputbuttonadd.innerHTML="+";
                  cart_inputbuttonadd.style="background:#4E4E4F; padding:auto;float:right;";
                  cart_inputbuttonmins.innerHTML="-";
                  cart_inputbuttonmins.style="background:#4E4E4F;padding:auto;float:left;";

                  var obj = document.getElementById("myTable").insertRow(-1);
                  var x=obj.insertCell(0)    //name c0
                  var y=obj.insertCell(1)    //quantity c1
                   var z=obj.insertCell(2)    //price c2
                   x.innerHTML=heading.textContent;
                   y.innerHTML=NumberInpute.value;
                   z.innerHTML=CalculatedPrice.toFixed(2);
                   var AAAprice=CalculatedPrice.toFixed(2);
                      y.appendChild(cart_inputbuttonmins);
                   y.appendChild(cart_inputbuttonadd);

                   var  Btnmins= function()
                   {
                 ...
                 //heading.textContent is product name
                 //Newroductquantity is quality
                //CalculatedPrice.toFixed(2) is productprice * quantity.


                 //remove input number and set it to none
                     NumberInpute.value="";

                      ShoppingCartTotalPrice =   Number(ShoppingCartTotalPrice) + Number(AAAprice);;

                      var ShowPrice = document.getElementById("TotalPrice");
                       ShowPrice.innerHTML='$'+ShoppingCartTotalPrice.toFixed(2);
cart_inputbuttonmins.onclick =Btnmins;
    ...
    // as 1.40, not 1.4.
    para.textContent = product.unitPrice.toFixed(1);
